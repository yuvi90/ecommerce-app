...
export const newProduct = TryCatch(async (req: Request, res, next) => {
  const newProductRequestBody: NewProductRequestBody = req.body;
  const { name, price, stock, category } = newProductRequestBody;
...
export const getAllProducts = TryCatch(async (req: Request, res) => {
  const searchQuery: SearchRequestQuery = req.query;
  const { search, sort, category, price } = searchQuery;
